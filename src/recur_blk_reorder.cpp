	...
	double *workDbl, int lworkDbl, int *workInt, int lworkInt)
{
	...
	for (int i = 0; i < nb; i++)
	{
		...
		if(minIdx != i) //switch blk order in B, UV, a, b, and oldIdx
		{
			...
			// swap lastRow
			swap_ranges(lastRow.data() + offset, lastRow.data() + 
				offset + m, lastRow.data() + minIdx * m);
		} // minIdx != i
		// uni_reorder again
		int offset = i*m;
		iota(idxj, idxj+m, 0);
		...
		// reorder lastRow
		reorder(lastRow.data() + offset, idxj, m, subworkInt, lsubworkInt);
		// reorder in i-th blk
		for(int j = 0; j < nb; j++)
		{
			if(j < i)
...
//				transpose();
		...
		// lastRow.segment(i * m, m) = lastRow.segment(i * m, m) * Bi^T;
		int incr = 1;
		F77_CALL(dtrmv)("L", "N", "N", &m, Bi, &m, lastRow.data() + i * m, 
			&incr);
		...
		// lastRow Schur
		for(int l = i + 1; l < nb; l++)
		{
			int idx = (2 * nb - 1 - i) * i / 2 + (l - 1 - i);
			RowVectorXd tmpRowVec = lastRow.segment(i * m, m) * 
				UV[idx].V.block(0, 0, m, UV[idx].crtColNum) *
				UV[idx].U.block(0, 0, m, UV[idx].crtColNum).transpose();
			lastRow.segment(l * m, m) -= tmpRowVec;
		}
		lastRow(n) -= lastRow.segment(i * m, m).squaredNorm();
	} // i = 0 : nb
	if(lastRow(n) <= 0.0)
		Rcpp::stop("Matrix inverse failed in recursive block "
			"reordering\n");
	lastRow(n) = sqrt(lastRow(n));
